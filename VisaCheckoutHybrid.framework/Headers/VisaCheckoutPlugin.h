...
/**
 ...
 A typical setup will add the following message handler names on the
 instance of WKWebView that will show Visa Checkout button.
 Before the page that renders the Visa Checkout button loads, the following
 code should be called.
 
 *  *  *  *  *
 
 ## Example of a typical setup required to launch Visa Checkout
 ````
 
 let controller = <#My WKWebView instance#>.configuration.userContentController
 controller.add(VisaCheckoutPlugin.main, name: VisaMessageHandler.configureVisaCheckoutPlugin)
 controller.add(VisaCheckoutPlugin.main, name: VisaMessageHandler.launchVisaCheckout)
 VisaCheckoutPlugin.main.presentingViewController = self
 ````
 Or if you prefer, VisaCheckoutPlugin can do this for you by calling 
 VisaCheckoutPlugin.configure(_:viewController:).
 */
