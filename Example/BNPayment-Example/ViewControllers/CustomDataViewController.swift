        ...
        if let data = jsonText?.data(using: String.Encoding.utf8) {
            ...
            do {
                guard let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String:Any] else {
                    throw NSError(domain: String(describing: self), code: 1, userInfo: [NSLocalizedDescriptionKey: "Content not a valid json"])
                }
                
                AppSettings.sharedInstance().persistJsonDataRegistration(json, forKey: key)
